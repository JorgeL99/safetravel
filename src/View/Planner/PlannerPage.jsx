import { FiAlertTriangle, FiArrowDown, FiArrowUp, FiClock, FiMap, FiNavigation, FiTrash2 } from 'react-icons/fi';
...
import { analyzeItinerary, buildItinerarySchedule, calculateTripBudget, optimizeItineraryRoute } from '../../lib/travel-utils';
...
const PlannerPage = () => {
  ...
  const transferDays = schedule.reduce((totalDays, item) => totalDays + (item.transfer?.transferDays ?? 0), 0);
  ...
  return <>
    ...
    <main className="collectionPage container section">
      ...
      <h1>Mi itinerario</h1>
      <p>Ordena tus actividades y obtén una referencia inmediata de tiempo, traslados y presupuesto.</p>
      {planned.length ? <div className="plannerLayout">
        <div>
          ...
          <ol className="plannerList">{schedule.map((destination, index) => <li key={destination.id}>
            ...
            <div>{destination.transfer && <span className="transferDetail"><FiNavigation /> {destination.transfer.distanceKm} km aprox. · {destination.transfer.mode}{destination.transfer.transferDays ? ' · 1 día de traslado' : ''}</span>}<small>{destination.province} · {destination.duration}</small><h2>{destination.name}</h2><strong>S/ {destination.price}</strong></div>
          ...
          <div className="scheduleSummary"><FiClock /><span>Duración total estimada</span><strong>{schedule.at(-1)?.endDay ?? 0} días</strong><small>Incluye {transferDays} {transferDays === 1 ? 'día' : 'días'} de traslado. Las distancias son lineales y deben confirmarse con el operador.</small></div>
        </div>
